                        ...
                        <form action="{{ route('quiz.update', $quiz) }}" method="post">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="name" id="name" class="form-control"
                                               value="{{ old('name', $quizProvider['name']) }}"
                                               required>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select name="category_id" id="category" class="form-control select2">
                                            @foreach($categories as $item)
                                                <option
                                                    value="{{ $item['id'] }}" {{ old('category_id', $quizProvider['category']['id']) == $item['id'] ? 'selected': '' }}>{{ $item['name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <div class="input-group">
                                                    <input type="text" class="date-time-picker form-control"
                                                           name="end_date"
                                                           data-options='{"timepicker":true, "format":"d-m-Y H:m"}'
                                                           value="{{ old('end_date', $quiz['end_date']) }}" required/>
                                                    <div class="input-group-append">
                                                        <span class="input-group-text add-on white">
                                                            <i class="icon-calendar"></i>
                                                        </span>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="password" id="password" class="form-control"
                                               value="{{ old('password', $quizProvider['password']) }}" required>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <textarea name="description" id="editor" class="form-control"
                                                  rows="5">{{ old('description', $quizProvider['description']) }}</textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="row border-top border-list mb-4 pt-3" id="question-area">
                                ...
                                @if(old('questions'))
                                                    ...
                                                    @foreach($question['options'] as $opKey => $option)
                                                        ...
                                                        <div class="col-md-11 mb-2">
                                                            <div class="input-group">
                                                                ...
                                                                @isset($option['id'])
                                                                    <input type="hidden" name="questions[{{$key}}][options][{{$opKey}}][id]" value="{{ $option['id'] }}">
                                                                @endisset
                                                                <textarea
                                    ...
                                    @endforeach
                                @else
                                    @foreach($quizProvider['questions'] as $key => $question)
                                        <div class="col-md-12 mb-2">
                                            <div class="form-group">
                                                <div class="input-group">
                                                    <input type="hidden" name="questions[{{$key}}][id]"
                                                        ...
                                                        <div class="col-md-11 mb-2">
                                                            <div class="input-group">
                                                                <div class="input-group-append">
                                                                    <div class="input-group-text">
                                                                        <div
                                                                            ...
                                                                            <input type="radio"
                                                                                   name="answer{{$key}}"
                                                                                   value="true" id="answer{{$option['id']}}"
                                                                                   required="required"
                                                                                   class="custom-control-input" {{ $question['answer']['id'] === $option['id'] ? 'checked': '' }}>
                                                                ...
                                                                <textarea
                                                                    ...
                                                                    type="text" required
                                                                    name="questions[{{$key}}][options][{{$opKey}}][content]">{{ $option['content'] }}</textarea>
                                                                @if($question['answer']['id'] === $option['id'])
                                                                    <input type="hidden" class="{{ $key }}"
                                                                           name="questions[{{$key}}][options][{{$opKey}}][answer]:boolean"
                                                                           value="true">
                                                                @endif
...
@push('js')
    ...
    <script>
        ...
        @if(old('questions'))
            counter = parseInt('{{ count(old('questions', $quizProvider['total_question'])) }}');
